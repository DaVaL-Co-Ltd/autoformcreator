      ...
      <nav className="flex-1 px-3 py-4 space-y-1 sidebar-scroll overflow-y-auto">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`p-1.5 rounded-lg transition-colors ${isActive ? 'bg-primary text-white shadow-md shadow-primary/30' : 'bg-sidebar-hover text-sidebar-text group-hover:text-white'}`}>
                <Icon size={16} strokeWidth={isActive ? 2.2 : 1.8} />
              </div>
              <div className="flex-1 min-w-0">
                <span className="block truncate">{label}</span>
                <span className={`block text-[10px] truncate ${isActive ? 'text-primary-light' : 'text-sidebar-text/50'}`}>{description}</span>
              </div>
