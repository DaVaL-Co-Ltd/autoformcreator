...
// 모델이 규칙을 어기고 끝내 한글을 그릴 경우라도, 깨진/뒤틀린 한글 대신
// 실제 콘텐츠 텍스트가 정확히 들어가도록 하는 마지막 안전장치 문구.
export function buildKnowledgeTextFallbackClause(exactText) {
  const clean = String(exactText || '').replace(/\s+/g, ' ').trim()
  if (!clean) return ''
  return ` LAST-RESORT TEXT RULE: drawing text is strongly discouraged, but if — and only if — some Korean text unavoidably ends up in the image, that text MUST read exactly and correctly as "${clean}". It must be real, perfectly spelled, fully readable Korean with correctly formed Hangul characters. Never render broken, garbled, distorted, scrambled, or invented Korean glyphs. Do not invent any other Korean words.`
}
