...
export const KNOWLEDGE_INSIGHT_CORNER_LAYOUT_PROMPT = 'Create a compact square illustration asset intended to be placed at the lower-right corner of a knowledge card. The subject must be SMALL: occupy roughly 30% to 40% of the canvas area, tucked into the lower-right corner inside a 45% by 45% region anchored at the lower-right edge. The remaining roughly 60% of the canvas (top half and left half) must stay completely empty white. Do not enlarge the subject to fill more space. Do not design a full poster, do not center the subject, do not fill the whole frame, and do not add a text panel, badge, frame, border, sticker sheet, decorative background scene, or any line along the canvas edges.'
