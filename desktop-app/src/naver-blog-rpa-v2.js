...
// 입력하고 ArrowLeft 로 커서를 단락 맨 앞으로 옮긴 뒤 이모지만 삽입한다. DOM Selection 변경은
// SmartEditor 내부 커서 모델이 무시하므로 반드시 키보드 방향키로 커서를 이동해야 한다.
...
async function insertParagraphText(page, rawText) {
  ...
  // 2) ArrowLeft 로 커서를 단락 맨 앞으로 이동 (키 이벤트는 SmartEditor 가 존중)
  for (let i = 0; i < caretCount; i += 1) {
    await page.keyboard.press('ArrowLeft')
  }
  // 3) 이모지만 삽입 — 입력 데이터가 공백이 아니라 이모지라 bullet 트리거 미발동
  await page.keyboard.insertText(emojiGlyphs)
  // 4) 커서를 단락 끝으로 복귀 (다음 블록 Enter 대비)
  for (let i = 0; i < caretCount; i += 1) {
